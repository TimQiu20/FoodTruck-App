...
register: BEGIN
	...
	IF i_email not in (NULL,'')
		THEN IF EXISTS( SELECT username from Employee WHERE email = i_email)
        THEN leave register;
        end if;
	END IF;

	IF i_username in (NULL, '') or i_firstname in (NULL, '') or i_lastname in (NULL, '') or i_password in (NULL, '') or length(i_password) < 8
		or EXISTS (select username from User where username = i_username)
		or EXISTS (select username from User WHERE (firstName, lastName) = (i_firstname, i_lastname))
		or (i_balance is NULL and i_type in (NULL,''))
		THEN leave register;
	END IF;
	insert into user(username, password, firstName,lastName) values (i_username, md5(i_password), i_firstname, i_lastname);

    IF i_balance > 0
		THEN insert into Customer values (i_username, i_balance, NULL);
    END IF;
    IF i_email <> '' and i_email is not NULL
         THEN insert into Employee (username, email) VALUE (i_username, i_email);
        IF i_type = 'Admin' THEN
        insert into Admin (username) VALUE (i_username);
        elseif i_type = 'Manager'THEN
        insert into Manager (username) VALUE (i_username);
        ELSEIF i_type = 'Staff' THEN
        insert into Staff (username, foodTruckName) VALUE (i_username, NULL);
        END IF;
    END IF;
END //
DELIMITER ;
...
BEGIN
    SET @partialName = i_foodTruckName;
...
SELECT FoodTruck.foodTruckName, FoodTruck.stationName, remain as remainingCapacity, COUNT(DISTINCT username) as staffCount, COUNT(DISTINCT foodName) as menuItemCount
    FROM FoodTruck
    INNER JOIN STATION
	...
	inner join (
    select Station.stationName, capacity - count(distinct(foodTruckName)) as remain from
    Station natural join FoodTruck
    group by Station.stationName
    ) as rem
    on Station.stationName = rem.stationName
    WHERE
    (i_managerUsername = managerUsername) AND
    (i_foodTruckName = FoodTruck.foodTruckName OR i_foodTruckName = "" OR FoodTruck.foodTruckName LIKE CONCAT('%', @partialName, '%')) AND
...
-- Everything is ok for this query
...
-- Fixed: check the Capacity before add station
...
19a: BEGIN
-- check capacity of the station
if  (select capacity from Station where stationName = i_stationName) - (select count(*) from foodTruck where stationName = i_stationName) <= 0 then leave 19a;
end if;
...
if (select foodTruckName from staff where i_staffName = username) is not null
then leave 19b;
end if;
...
-- Fixed: change the label of CONCAT(Fname, ' ', Lname)
...
-- Fixed: should use INSERT INTO before SELECT
...
-- Fixed: should check capacity before change the station
...
22a: BEGIN
	...
	if  (select capacity from Station where stationName = i_stationName) - (select count(*) from foodTruck where stationName = i_stationName) <= 0 then leave 22a;
	end if;
...
BEGIN
    ...
    SELECT foodTruckName, totalOrder, IFNULL(totalRevenue,0), totalCustomer
    FROM
...
-- Everything is ok with this query
...
-- Fixed: add '' case in WHERE clause
...
BEGIN
    ...
    SELECT Customer.stationName, Building.buildingName, GROUP_CONCAT(DISTINCT(buildingtag.tag) SEPARATOR ',') AS tags, Building.description, Customer.balance
	FROM Customer
	INNER JOIN Station ON customer.stationName = Station.stationName
	INNER JOIN Building ON building.buildingName = Station.buildingName
...
BEGIN
	...
	SELECT A.foodTruckName, CONCAT(USER.firstName , ' ' , USER.lastName) as managerName, A.foodNames FROM
	(SELECT foodtruck.foodTruckName, foodtruck.stationName, foodtruck.managerUsername, GROUP_CONCAT(menuitem.foodname SEPARATOR ',') AS foodNames
	FROM foodtruck
	INNER JOIN menuitem ON foodtruck.foodTruckName = menuitem.foodTruckName
	GROUP BY foodTruckName) AS A
	INNER JOIN customer ON customer.stationName = A.stationName
	INNER JOIN USER on USER.username = A.managerUsername
	WHERE customer.username = i_customerUsername;
...
BEGIN
    IF i_customerUsername IN (SELECT username FROM Customer) THEN
        IF (SELECT balance FROM Customer WHERE username = i_customerUsername) > 0 THEN
...
BEGIN
    SELECT @unit_price:=price FROM MenuItem
    WHERE STRCMP(i_foodTruckName, MenuItem.foodTruckName) = 0 AND STRCMP(i_foodName, MenuItem.foodName) = 0;
    SET @total_price = @unit_price * i_purchaseQuantity;

    SELECT @cus_username:=customerUsername FROM Orders
    WHERE i_orderID = Orders.orderID;
    SELECT @cus_balance:=balance FROM Customer
    WHERE @cus_username = Customer.username;

    IF
		@cus_balance > @total_price
	THEN
		UPDATE Customer
        SET balance = @cus_balance - @total_price
        WHERE @cus_username = Customer.username;
...
BEGIN
 ...
 SELECT Orders.date AS `Date`, Orders.orderID,
    SUM(MenuItem.price * OrderDetail.purchaseQuantity) AS 'Order Total',
    GROUP_CONCAT(Distinct(OrderDetail.foodName) SEPARATOR ',') AS foodNames,
    SUM(OrderDetail.purchaseQuantity) AS 'Food Quantity'
  From Orders natural join OrderDetail natural join MenuItem
        WHERE
